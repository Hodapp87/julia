...
@testset "LazyLibrary" begin; mktempdir() do dir
    ...
    global libccalllazyfoo = LazyLibrary(lclf_path, default_rtld_flags, LazyLibrary[], hdl -> global lclf_loaded = true)
    global libccalllazybar = LazyLibrary(lclb_path, default_rtld_flags, [libccalllazyfoo], hdl -> global lclb_loaded = true)
    ...
    @test dlpath(libccalllazybar) == realpath(libccalllazybar.name)
    @test lclf_loaded
    close_libs()
end; end
